...
def parse_game(game):
    ...
    pitch_sql = "INSERT INTO raw_pitch (" + ",".join(pitch_fields[key] for key in sorted(pitch_fields.keys())) + ",enhanced,balls,strikes) VALUES (" + ",".join(map(lambda a: '?', pitch_fields.keys())) + ",?,?,?)"
    atbat_inserts = []
    pitch_inserts = []
    for xml_file in xml_files:
        ...
        for atbat in atbats:
            atbat_inserts.append([atbat.get(key).strip() for key in sorted(atbat_fields.keys())])
    ...
    conn.executemany(atbat_sql, atbat_inserts)
